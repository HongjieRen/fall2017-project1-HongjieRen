...
```{r}
heatmap.2(cor(sentence.list%>%filter(type=="inaug",File=="BarackObama")%>%select(anger:trust)), 
          scale = "none", 
...
heatmap.2(cor(sentence.list%>%filter(type=="farewell",File=="BarackObama")%>%select(anger:trust)),          scale = "none", 
          col = bluered(100), , margin=c(6, 6), key=F,
          trace = "none", density.info = "none")
